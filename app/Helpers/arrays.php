...
function arrange($id = null, $coll = [],$autoformat = false){
    if(!$autoformat) {
...
if (! function_exists('hArraysTest')) {
   function hArraysTest($id = null, $autoformat = false) {
       ...
       return arrange($id,$coll,$autoformat);
   }
}
    ...
    function array_clear($array = [], $strict = false)
    {
        ...
        if ($strict)
            return array_filter($values, function ($item) {
                return !in_array($item,[0,0.0,'0','No']);
            });
        return $values;
...
if (! function_exists('w2_array_tipo_alquiler')) {
   function w2_array_tipo_alquiler($id = null, $autoformat = false) {
       $coll = [
           '1'  => 'Anual',
           '2'  => 'Invernal',
           '3'  => 'Temporada',
